...
// Emit selected signature image
function useSignature() {
  console.log('Emitting selected signature:', currentSelect.value);
  if (currentSelect.value) {
    emit('useSignature', currentSelect.value); // <- emits actual base64 image
    close();
  }
}
...
const emit = defineEmits(['useSignature', 'bulkSign']);
const props = defineProps<{
  documents?: { docId: string; fileUrl: string }[];
}>();
...
function useSignature() {
  ...
  // ✅ NEW: If documents were passed, emit bulk event
  if (props.documents?.length) {
    ...
    emit('useSignature', signaturePayload);
  }
